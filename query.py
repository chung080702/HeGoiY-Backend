...
LEFT JOIN
(
    SELECT hotel_id, sum(score * owa_weight.v) as room_score, GROUP_CONCAT(dd.id) as roomIds
    FROM 
    (
        SELECT 
            aa.id as id, 
...
ON ee.hotel_id = rnk.hotel_id
LEFT JOIN
(
	SELECT * FROM hotels h2
) fff
ON fff.id = rnk.hotel_id
LEFT JOIN
(
    SELECT h.id, GROUP_CONCAT(hf.name) as service_names  FROM hotels h, hotel_feature_relations hfr, hotel_features hf  
    WHERE h.id = hfr.hotel_id AND hf.id = hfr.feature_id AND hfr.feature_id in (:request_hotel_feature)
    GROUP BY h.id
) kkk
